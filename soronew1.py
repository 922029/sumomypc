...
if __name__ == "__main__":
    ...
    for tl_id in TL_IDS:
        ...
        # 手動学習ループを使うため、内部スケジューラ用変数はデフォルトのまま、あるいは未使用

    all_episode_wait_times = {tl: [] for tl in TL_IDS}
    
    # Epsilon-Greedyの設定
    # 開始時の探索率 1.0 から全エピソードの 80%の間に終了時の探索率 0.05 まで下げる
    EPSILON_START = 1.0
    EPSILON_END = 0.05
    DECAY_RATIO = 0.8
    DECAY_EPISODES = int(EPISODES * DECAY_RATIO)
    
    for ep in range(EPISODES):
        # 現在のEpsilonを計算 (線形減衰)
        if ep < DECAY_EPISODES:
            # 進行度合いに応じて 1.0 -> 0.05 へ減衰
            epsilon = EPSILON_START - (ep / DECAY_EPISODES) * (EPSILON_START - EPSILON_END)
        else:
            # 80%以降は固定
            epsilon = EPSILON_END
            
        print(f"Episode {ep+1}/{EPISODES} Start. Current Epsilon: {epsilon:.4f}")
        ...
        while not done:
            ...
            for tl in TL_IDS:
                ...
                if np.random.rand() < epsilon:
                    # 探索: ランダムな行動を選択
                    actions[tl] = env.action_space.sample()
                else:
                    # 活用: モデルの推論結果 (deterministic=True) を選択
                    actions[tl] = int(agents[tl].predict(obs[tl], deterministic=True)[0])
